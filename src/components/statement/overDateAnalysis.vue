<template>
  <div class="overDate">
    <div class="left-tab" :class="{ active: isactive }" @click="jump('analysis')"><span>认购未签约(逾期)</span></div>
    <div class="right-tab" :class="{ active: isactive2 }" @click="jump('qianYueAnalysis')"><span>签约未回款(逾期)</span></div>
    <clear></clear>
    <div class="top-title">
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        isactive: this.$store.state.statementAnalysis.isactive,
        isactive2: this.$store.state.statementAnalysis.isactive2,
        listshown: false,
    ...
    methods: {
      ...
      jump(router) {
        ...
        this.$store.commit('statementAnalysis', statementAnalysis)
        this.$router.replace('/statement/' + router)
      },
      jumpProject() {
        if (this.isactive === true) {
          this.$router.push('/statement/selectProject/0/analysis')
        } else {
          this.$router.push('/statement/selectProject/0/qianYueAnalysis')
        }
      }
    }
  }
