<template>
	<div>
    <ul v-if="parent == '0' && projects.length > 0">
      <li class="gzb-list-item">
        <div class="gzb-left-content" @click="checkAll()">
          全部项目
        </div>
        <clear></clear>
      </li>
...
export default {
  ...
  methods: {
    ...
    async LToChild(pIndex, index) {
      let statementProject = this.$store.state.statementProject
      let frights = this.$store.state.fright
      let check = false
      statementProject.pk_project = this.projects[pIndex].contents[index].pkProject
      statementProject.name = this.projects[pIndex].contents[index].names
      statementProject.pk_responsecorp = this.projects[pIndex].contents[index].pk_responsecorp
      statementProject.vcode = this.projects[pIndex].contents[index].code
      if (this.projects[pIndex].contents[index].parentCode === '0') {
          statementProject.outvcode = this.projects[pIndex].contents[index].code
        }
      let params = {
      ...
      if (!check) {
        statementProject.fright = ''
      }
      await defaultProject(params)
      this.$store.commit('statementProject', statementProject)
      if (this.type === 'overDateQianYue') {
        this.$router.replace('/statement/overDate/' + this.type)
      } else {
        this.$router.replace('/statement/' + this.type)
      }
    },
    RToChild(pIndex, index) {
      ...
      if (this.projects[pIndex].contents[index].childSize > 0) {
        this.$router.push('/statement/selectProject/' + this.projects[pIndex].contents[index].code + '/' + this.type)
      }
      this.emptyProject()
      // if (this.projects[pIndex].contents[index].childSize === 0) {
      //   let selectedProject = {
