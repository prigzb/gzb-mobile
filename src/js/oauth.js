import config from './mcloud.config';
...
export default {
   ...
   * @returns {*}
   */
  getAccessToken: async function (appId, appSecret) {
    let version = "1.1";
    let appid = appId;
    ...
    let oauthurl = config.mcloud.urlPrefix + '/openauth2/api/token?grant_type=client_credential&appid=' + appid + '&secret=' + secret;
   ...
   */
  sendRequest: async function(url,token,ticket){
    ...
    //发送http请求，并处理返回结果
    let res = await fetch( config.mcloud.urlPrefix + url + '?ticket=' + ticket + '&access_token=' + token, {
      method: 'GET',
    });
    let resData = await res.json();
    return resData;
